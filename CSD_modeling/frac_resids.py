...
for i in range(len(targets)):
    ...
    if np.logical_or(targets[i] == 'HD143006', targets[i] == 'HD163296'):
        dhdu = fits.open('data/deep_'+targets[i]+'_data_symm.JvMcorr.fits')
        rhdu = fits.open('data/deep_'+targets[i]+'_resid_symm.JvMcorr.fits')
    else:
        dhdu = fits.open('data/deep_'+targets[i]+'_data.JvMcorr.fits')
        rhdu = fits.open('data/deep_'+targets[i]+'_resid.JvMcorr.fits')
    ...
    frac_ = 100 * (rim / dim)
    print(targets[i], 1e6 * np.max(dim) / disk.disk[targets[i]]['RMS'])
    frac  = np.zeros_like(rbins)
    lfrac = np.zeros_like(rbins)
    hfrac = np.zeros_like(rbins)
    for ir in range(len(rbins)):
        inbin = ((r >= rbins[ir]-0.5*dr) & (r < rbins[ir]+0.5*dr))
        frac[ir] = np.average(frac_[inbin])
        lfrac[ir] = np.min(frac_[inbin])
        hfrac[ir] = np.max(frac_[inbin])
    ...
    #                edgecolor='none')
    ax.plot([0, 1.2], [0, 0], '-g')
    ax.plot([0, 1.2], [-10, -10], ':g')
    ax.plot([0, 1.2], [10, 10], ':g')
    ax.plot(rbins, frac, 'k')
    ax.plot(rbins, lfrac, 'b')
    ax.plot(rbins, hfrac, 'r')
    ...
    for ir in range(len(rgap)):
        ax.axvspan(rgap[ir] - wgap[ir], rgap[ir] + wgap[ir], 
