...
disk_name = ['dx5mas', 'dy5mas', 'incl_2lo', 'incl_2hi', 'PA_5lo', 'PA_5hi',
             'dx5_incl2', 'dx5_PA5', 'incl2_PA5', 'incl2_PA5_dx5']
disk_lbls = ['$\\Delta x$=+5', '$\\Delta y$=+5', '$\\Delta i$=$-$2', 
...
plt.rc('font', size=7)
left, right, bottom, top = 0.10, 0.85, 0.05, 0.99
...
for i in range(len(disk_name)):
    ...
    if (i == 8):
        ...
        ax.set_ylabel('DEC offset  ($^{\prime\prime}$)', labelpad=1)
        ax.set_xticks([1, 0, -1])
        ax.set_yticks([-1, 0, 1])
    else:
        ax.set_xticks([])
...
# colorbar
cbax = fig.add_axes([right + 0.01, bottom, 0.02, 0.373])
...
fig.savefig('../../figs/geom_test1.pdf')
